...
function selectReverseProvider(options) {
    ...
    const requested = (raw === 'serpapi' ? 'serpapi-lens' : raw);
...
class ImageResolver {
    ...
    config;
    session;
    constructor(ctx, config, session) {
        this.ctx = ctx;
        this.config = config;
    ...
    async store(buffer, filename, mime, candidate) {
        return (0, cache_1.storeManagedImage)(this.ctx, this.config, buffer, filename, mime, {
            ...(candidate ? {
                kind: 'keyword-search',
                originalUrl: candidate.url,
                sourcePage: candidate.sourcePage,
...
class ReverseImageResolver {
    ...
    async resolve(imageUrl, providerOverride, maxResultsOverride) {
        ...
        const manifest = await (0, cache_1.findManagedCacheByCachedUrl)(directory, imageUrl).catch(() => undefined);
        const publicOriginalUrl = manifest?.originalUrl && (0, utils_1.isPublicHttpUrl)(manifest.originalUrl) ? manifest.originalUrl : undefined;
        const publicImageUrl = publicOriginalUrl || (0, utils_1.rewriteImageUrlForPublicAccess)(imageUrl, this.ctx.server?.selfUrl || '', this.config.reverse.publicBaseUrl || this.config.delivery.publicBaseUrl);
        ...
        const tryProvider = async (provider) => {
            if (provider === 'google')
                return this.callGoogleVision(imageUrl, maxResults);
        ...
        try {
            ...
            primaryError = (0, utils_1.formatError)(error);
        }
        if (selected.fallback && !explicitOverride) {
            try {
                ...
                return {
                    ...
                    fallbackError: (0, utils_1.formatError)(fallbackError),
                    hint: `Both providers failed. ${selected.provider}: ${primaryError}. ${selected.fallback}: ${(0, utils_1.formatError)(fallbackError)}.`
                };
            }
        }
        return {
    ...
    async callSerpApi(imageUrl, maxResults) {
        ...
        const publicImageUrl = (0, utils_1.rewriteImageUrlForPublicAccess)(imageUrl, this.ctx.server?.selfUrl || '', this.config.reverse.publicBaseUrl || this.config.delivery.publicBaseUrl);
    ...
    async cacheReverseInput(imageUrl, provider) {
        try {
            const directory = (0, node_path_1.join)(this.ctx.baseDir, this.config.storage.localDirectory);
            const existing = await (0, cache_1.findManagedCacheByCachedUrl)(directory, imageUrl).catch(() => undefined);
