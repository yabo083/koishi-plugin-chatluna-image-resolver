...
exports.markManagedCacheEntryChecked = markManagedCacheEntryChecked;
exports.listManagedImageCache = listManagedImageCache;
exports.findManagedCacheByOriginalUrl = findManagedCacheByOriginalUrl;
exports.findManagedCacheByCachedUrl = findManagedCacheByCachedUrl;
exports.searchManagedCache = searchManagedCache;
exports.sweepManagedCacheOriginalUrls = sweepManagedCacheOriginalUrls;
...
exports.writeManagedAssetManifest = writeManagedAssetManifest;
...
async function markManagedCacheEntryChecked(directory, manifestName, check, now = Date.now()) {
    if (!isManagedCacheFilename(manifestName) || !manifestName.endsWith('.json')) {
        throw new Error('invalid managed manifest name');
    }
    const file = (0, node_path_1.join)(directory, manifestName);
...
async function findManagedCacheByOriginalUrl(directory, originalUrl) {
    if (!originalUrl)
        return undefined;
...
async function findManagedCacheByCachedUrl(directory, cachedUrl) {
    if (!cachedUrl)
        return undefined;
    const { items } = await listManagedImageCache(directory);
    return items.find((item) => item.url === cachedUrl && !item.originalUrlExpired);
}
async function searchManagedCache(directory, query, limit = 10) {
    if (!query)
        return [];
    const { items } = await listManagedImageCache(directory);
...
async function sweepManagedCacheOriginalUrls(directory, config, options = {}) {
    const now = options.now ?? Date.now();
    ...
    for (const item of candidates) {
        ...
        if (result.ok === false) {
            ...
            if (config.storage.autoRevive && item.filename) {
                ...
                if (assetMissing) {
                    try {
                        const kind = (0, utils_1.detectManagedAssetKind)(item.filename, item.mime || '');
...
async function isAssetMissing(directory, item) {
    if (!item.filename)
        return false;
    try {
        await (0, promises_1.stat)((0, node_path_1.join)(directory, item.filename));
        return false;
    }
    catch {
        return true;
