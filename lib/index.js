...
const REVERSE_TOOL_SCHEMA = zod_1.z.object({
    ...
    provider: zod_1.z.enum(['auto', 'serpapi', 'serpapi-lens', 'google']).optional().describe('Override the configured reverse-search provider for this call. Auto chooses Google for private/local URLs and Google Lens for public QQ/Tencent CDN URLs.'),
...
exports.Config = koishi_1.Schema.intersect([
    ...
    koishi_1.Schema.object({
        reverse: koishi_1.Schema.object({
            ...
            description: koishi_1.Schema.string().role('textarea').default('Reverse-searches an image with automatic provider selection: Google Vision for private/local images that require base64 bytes, and SerpApi Google Lens for public QQ/Tencent CDN image URLs.').description('以图搜图工具描述。'),
            provider: koishi_1.Schema.union([
                koishi_1.Schema.const('auto').description('自动选择：私有/本地 URL 且配置了 Google Key 时走 Google Vision；公网 QQ/Tencent CDN 图片优先走 SerpApi Google Lens。'),
