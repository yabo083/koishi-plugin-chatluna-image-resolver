...
const QQ_MEDIA_CACHE_LOOKUP_SCHEMA = zod_1.z.object({
    messageId: zod_1.z.string().optional().describe('QQ/OneBot message id that contains media or a file. If omitted, recent tracked media messages are returned.'),
    ...
    imageIndex: zod_1.z.number().int().min(0).optional().describe('Backward-compatible image index alias. When provided, kind defaults to image and this value is used as mediaIndex.'),
    kind: zod_1.z.enum(['image', 'audio', 'text', 'file']).optional().describe('Optional media kind filter.'),
    limit: zod_1.z.number().int().min(1).max(20).optional().describe('How many recent media messages to list when messageId is omitted. Defaults to 5.')
});
