...
exports.usage = exports.Config = exports.selectReverseProvider = exports.trimTrailingSlash = exports.serpApiReversePayloadToResult = exports.serpApiLensPayloadToResult = exports.serpApiImagesToCandidates = exports.rewriteUrlBase = exports.rewriteImageUrlForPublicAccess = exports.mimeFromFilename = exports.isPublicHttpUrl = exports.detectManagedAssetKind = exports.buildSerpApiReverseImageUrl = exports.buildSerpApiImagesUrl = exports.buildSerpApiGoogleLensUrl = exports.buildGoogleVisionWebDetectionUriRequest = exports.buildGoogleVisionWebDetectionRequest = exports.storeManagedAsset = exports.sweepManagedCacheOriginalUrls = exports.searchManagedCache = exports.markManagedCacheEntryExpired = exports.markManagedCacheEntryChecked = exports.listManagedImageCache = exports.isManagedCacheFilename = exports.findManagedCacheByOriginalUrl = exports.findManagedCacheByCachedUrl = exports.cleanupManagedImageCache = exports.checkRemoteImageAlive = exports.inject = exports.name = void 0;
...
exports.inject = { optional: ['chatluna', 'server', 'console'] };
...
Object.defineProperty(exports, "findManagedCacheByCachedUrl", { enumerable: true, get: function () { return cache_2.findManagedCacheByCachedUrl; } });
Object.defineProperty(exports, "findManagedCacheByOriginalUrl", { enumerable: true, get: function () { return cache_2.findManagedCacheByOriginalUrl; } });
...
Object.defineProperty(exports, "markManagedCacheEntryChecked", { enumerable: true, get: function () { return cache_2.markManagedCacheEntryChecked; } });
...
Object.defineProperty(exports, "searchManagedCache", { enumerable: true, get: function () { return cache_2.searchManagedCache; } });
Object.defineProperty(exports, "sweepManagedCacheOriginalUrls", { enumerable: true, get: function () { return cache_2.sweepManagedCacheOriginalUrls; } });
...
Object.defineProperty(exports, "trimTrailingSlash", { enumerable: true, get: function () { return utils_2.trimTrailingSlash; } });
...
const REVERSE_TOOL_SCHEMA = zod_1.z.object({
    ...
    provider: zod_1.z.enum(['auto', 'serpapi-lens', 'google']).optional().describe('Override the reverse-search provider. Auto tries the best provider first and falls back to the other on failure.'),
...
const QQ_MEDIA_CACHE_LOOKUP_SCHEMA = zod_1.z.object({
    messageId: zod_1.z.string().optional().describe('QQ/OneBot message id that contains media. Also works for quoted/replied messages — pass the quoted messageId to find cached copies of expired images.'),
    mediaIndex: zod_1.z.number().int().min(0).optional().describe('Zero-based media index in the message. Defaults to all matching media.'),
    imageIndex: zod_1.z.number().int().min(0).optional().describe('Backward-compatible image index alias. When provided, kind defaults to image and this value is used as mediaIndex.'),
    kind: zod_1.z.enum(['image', 'audio', 'text', 'file']).optional().describe('Optional media kind filter.'),
    limit: zod_1.z.number().int().min(1).max(50).optional().describe('Max items to return. Defaults to 5 for tracker mode, 10 for recent/listCache.'),
    url: zod_1.z.string().optional().describe('Search cached manifests by URL (matches both cachedUrl and originalUrl). Works even if the original CDN link has expired — the URL string is still stored in manifests.'),
    listCache: zod_1.z.boolean().optional().describe('List recent cached manifests regardless of QQ tracker state.'),
    recent: zod_1.z.boolean().optional().describe('Return the most recent cached items sorted by time, with full user context. Equivalent to listCache=true with time ordering.'),
    userId: zod_1.z.string().optional().describe('Filter by userId. Use when a user asks for "my" previously cached media, e.g. "send me the images you found for me last time".')
});
exports.Config = koishi_1.Schema.object({
    ...
    reverseSearch: koishi_1.Schema.object({
        provider: koishi_1.Schema.union([
            koishi_1.Schema.const('auto').description('自动：优先尝试最佳提供方，失败后自动切换到另一种。'),
            koishi_1.Schema.const('serpapi-lens').description('SerpApi Google Lens：使用公网图片 URL，适合 QQ/NapCat CDN 链接。'),
            koishi_1.Schema.const('google').description('Google Cloud Vision：下载图片转 base64 提交，支持私有/本地 URL。')
    ...
    storage: koishi_1.Schema.object({
        ...
        localDirectory: koishi_1.Schema.string().default('data/chatluna-image-resolver').description('本地缓存目录，相对 Koishi baseDir。'),
        localPublicPath: koishi_1.Schema.string().default('/chatluna-image-resolver').description('本地缓存 HTTP 路径。'),
        expiredRetentionMinutes: koishi_1.Schema.number().min(1).max(24 * 365 * 60).default(5).description('原始直链检测失效后，缓存资源继续保留多少分钟再清理。默认 5 分钟。'),
        cleanupIntervalMinutes: koishi_1.Schema.number().min(1).max(24 * 30 * 60).default(5).description('统一缓存清理间隔分钟数。默认 5 分钟。'),
        livenessCheckBatchSize: koishi_1.Schema.number().min(1).max(100).default(12).description('每轮自动巡检最多检查多少条原始直链。用于控制网络开销。'),
        autoRevive: koishi_1.Schema.boolean().default(true).description('本地缓存文件丢失但原始直链仍存活时，自动重新下载并恢复缓存。'),
...
function daysFromMinutes(minutes, fallbackDays) {
...
class ImageResolverTool extends tools_1.StructuredTool {
    ...
    async _call(input) {
        ...
        const resolver = new resolvers_1.ImageResolver(this.ctx, this.config, session);
...
class ReverseImageResolverTool extends tools_1.StructuredTool {
    ...
    async _call(input) {
        const session = this.tracker.getSessionContext();
        const resolver = new resolvers_1.ReverseImageResolver(this.ctx, this.config, session);
...
class QQMediaCacheLookupTool extends tools_1.StructuredTool {
    ctx;
    config;
    tracker;
    ...
    schema = QQ_MEDIA_CACHE_LOOKUP_SCHEMA;
    constructor(ctx, config, tracker) {
        super({});
        this.ctx = ctx;
        this.config = config;
    ...
    async _call(input) {
        const directory = (0, node_path_1.join)(this.ctx.baseDir, this.config.storage.localDirectory);
        if (input.url) {
            const url = input.url.trim();
            const byCached = await (0, cache_1.findManagedCacheByCachedUrl)(directory, url).catch(() => undefined);
            const byOriginal = byCached ? undefined : await (0, cache_1.findManagedCacheByOriginalUrl)(directory, url).catch(() => undefined);
            ...
            const fuzzy = await (0, cache_1.searchManagedCache)(directory, url, input.limit ?? 5).catch(() => []);
            if (fuzzy.length > 0) {
                return JSON.stringify({
                    ok: true,
                    mode: 'url-fuzzy',
        ...
        if (input.listCache || input.recent) {
            ...
            const { items: allItems } = await (0, cache_1.listManagedImageCache)(directory);
            const filtered = allItems
                .filter((item) => !item.originalUrlExpired)
                .filter((item) => !input.kind || (0, utils_1.detectManagedAssetKind)(item.filename || '', item.mime || '') === input.kind)
        ...
        const records = input.messageId
            ? [this.tracker.findMedia(input.messageId, kind)?.record].filter(Boolean)
        ...
        for (const record of records) {
            ...
            const selectedIndex = typeof mediaIndexInput === 'number'
                ? (0, utils_1.clamp)(mediaIndexInput, 0, Math.max(0, candidates.length - 1))
                : undefined;
            const selected = typeof selectedIndex === 'number' ? candidates.slice(selectedIndex, selectedIndex + 1) : candidates;
            for (const media of selected) {
                const cached = await (0, cache_1.findManagedCacheByOriginalUrl)(directory, media.src).catch(() => undefined);
                items.push({
...
function apply(ctx, input) {
    ...
    });
    {
        const runCacheMaintenance = async () => {
            const directory = (0, node_path_1.join)(ctx.baseDir, config.storage.localDirectory);
            await (0, cache_1.sweepManagedCacheOriginalUrls)(directory, config, {
                ...
                async onRevive(item, downloaded) {
                    ...
                    await (0, promises_1.writeFile)((0, node_path_1.join)(directory, item.filename), downloaded.buffer);
                    ...
                    const publicUrl = (0, utils_1.rewriteUrlBase)(localUrl, config.delivery.publicBaseUrl);
                    await (0, cache_1.writeManagedAssetManifest)(ctx, config, item.filename, publicUrl, downloaded.mime, downloaded.buffer.length, {
                        storage: 'local',
                        kind: item.kind,
                        originalUrl: item.originalUrl,
                        sourcePage: item.sourcePage,
                    ...
                    ctx.logger(exports.name).info('revived cache (local): %s -> %s', item.filename, publicUrl);
                }
            });
            await (0, cache_1.cleanupManagedImageCache)(directory, {
        ...
        ctx.inject(['server'], (ctx2) => {
            ...
            ctx2.server.post?.(`${config.storage.localPublicPath}/_cache/delete`, async (koa) => {
                const body = await (0, cache_1.readJsonBody)(koa);
                const manifests = Array.isArray(body?.manifests) ? body.manifests.filter((m) => typeof m === 'string' && /^[a-zA-Z0-9._-]+\.json$/.test(m)) : [];
                const directory = (0, node_path_1.join)(ctx.baseDir, config.storage.localDirectory);
                let deleted = 0;
                for (const manifest of manifests) {
                    try {
                        const manifestData = JSON.parse(await (0, promises_1.readFile)((0, node_path_1.join)(directory, manifest), 'utf8'));
                        const asset = typeof manifestData.filename === 'string' ? (0, node_path_1.join)(directory, manifestData.filename) : '';
                        if (asset) {
                            try {
                                await (0, promises_1.unlink)(asset);
                                deleted++;
                            }
                            catch { }
                        }
                        await (0, promises_1.unlink)((0, node_path_1.join)(directory, manifest));
                        deleted++;
                    }
                    catch { }
                }
        ...
        ctx.on('ready', () => {
            void runCacheMaintenance().catch((error) => ctx.logger(exports.name).warn('media cache maintenance failed: %s', (0, utils_1.formatError)(error)));
        });
        ctx.setInterval?.(() => {
            void runCacheMaintenance().catch((error) => ctx.logger(exports.name).warn('media cache maintenance failed: %s', (0, utils_1.formatError)(error)));
    ...
    const registerTool = (ctx2) => {
        ...
        if (config.qqMedia.enabled) {
            ...
            ctx2.logger(exports.name).info('registered ChatLuna QQ media cache lookup tool: qq_media_cache_lookup');
